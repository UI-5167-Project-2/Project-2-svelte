...
<div class="number-line-container">
  <div class="d-flex flex-row align-items-center justify-content-between gap-1">
    {#if switchEnable}
      <CheckboxInput bind:value={showRange} {label} {disabled} />
    {:else}
      {label}
    {/if}

    {#if disableButton}
      <div class="alert alert-warning" role="alert">Time ranges overlap</div>
    {/if}
  </div>
  {#if showRange}
    {#if switchEnable}
    ...
    <div class="number-line-wrapper">
      <!-- Number line -->
      <div class="number-line" bind:this={numberLineRef}>
        <!-- Hour markers -->
        {#each Array(25) as _, i}
          {@const leftPercent = 1 + (i / 24) * 98}
          <div class="hour-marker" style="left: {leftPercent}%">
    ...
    <div class="controls mt-3">
      {#each ranges as range, index}
        ...
        <div class="range-control">
          ...
          <button class="btn btn-sm btn-outline-danger" onclick={() => removeRange(index)}>
            Remove
          </button>
        </div>
      {/each}
      ...
      {#if ranges.length < 2}
        <button class="btn btn-sm btn-primary" onclick={addRange}> Add Time Range </button>
      {/if}
    </div>
  {/if}
</div>
